...
function PaginationLink({ children, enabled, href }) {
  if (!enabled) {
    return (
      <span href={href} className="border cursor-not-allowed  px-2 py-1 rounded text-slate-300 text-sm">
        {children}
      </span>
    );
  }
  return (
    <Link
      href={href}
      className="border flex  gap-1  item-center px-2 py-1 rounded text-slate-500 text-sm hover:bg-orange-100 hover:text-slate-700"
    >
      {children}
    </Link>
  );
